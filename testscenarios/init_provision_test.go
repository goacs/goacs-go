...
// recursive GetParameterNames/GetParameterValues walk of the whole device
// model (acs/methods/informmethods.go), and instead run the curated "init"
// script, which reads only DeviceInfo. and ManagementServer. via the
// blocking getParameterValues() bridge function and persists them via
// saveDevice().
...
func TestInitProvisionCuratedDiscovery(t *testing.T) {
	...
	if strings.Contains(res.Stdout, "<cwmp:GetParameterNames") {
		t.Fatalf("a brand-new device's first Inform should no longer trigger a recursive GetParameterNames walk; stdout:\n%s", res.Stdout)
	}
